...
from DDMS.settings import MEDIA_ROOT, MEDIA_URL
import json
from django.contrib import messages
from django.contrib.auth.models import User
from django.http import HttpResponse
from fmsApp.forms import UserRegistration, SavePost, UpdateProfile, UpdatePasswords
from fmsApp.models import Post,UserProfile
from .models import Department,File_type  # Import the Department model
...
def home(request):
    ...
    if request.user.is_superuser and request.user.is_staff:
        ...
        user_department = request.user.userprofile.userdepartment
        posts = Post.objects.filter(department=user_department).all()
    else:
...
def manage_post(request, pk=None):
    ...
    file_type=File_type.objects.all()
    if not pk is None:
        post = Post.objects.get(id=pk)
        context['post'] = post
    context['departments'] = departments  # Pass departments to the context
